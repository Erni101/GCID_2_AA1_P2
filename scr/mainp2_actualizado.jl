...
const DATA_PATH = joinpath(@__DIR__, "datos.csv")
const OUTPUT_DIR = joinpath(@__DIR__, "test")
const INDICES_PATH = joinpath(@__DIR__, "indices_cv.csv")
...
function round_numeric_df(df::DataFrame; digits::Int=4)
    ...
    for col in names(df2)
        if eltype(df2[!, col]) <: Real
            df2[!, col] = round.(df2[!, col], digits=digits)
        end
    end
...
function run_exercise_4()
    ...
    for model_name in ["ANN", "DoME", "SVM", "DecisionTree", "kNN"]
        ...
        push!(best_rows, (
            ...
            AccuracySTD = best.AccuracySTD,

            ErrorMedia = best.ErrorMedia,
            ErrorSTD = best.ErrorSTD,

            RecallMedia = best.RecallMedia,
            RecallSTD = best.RecallSTD,

            SpecificityMedia = best.SpecificityMedia,
            SpecificitySTD = best.SpecificitySTD,

            PrecisionMedia = best.PrecisionMedia,
            PrecisionSTD = best.PrecisionSTD,

            NPVMedia = best.NPVMedia,
            NPVSTD = best.NPVSTD,
        ...
        println("\n", model_name, ":")
        pretty_table(round_numeric_df(first(df_model, min(5, nrow(df_model))), digits=4))
    ...
    println("==========================")
    pretty_table(round_numeric_df(best_df, digits=4))
